...
\begin{document}
  ...
  While there was hope to reuse the \acs{JDBC} implementation of the \textit{repositories}, it was clear that at some point someone might want to use a \acs{NoSQL} database or some other means of storage. As explained in section \ref{subsec:jdbc} \nameref{subsec:jdbc}, an Android specific implementation unexpectedly came first.
  ...
  \attachlisting{snippets/build.gradle}
...
For Android clients you'll need to use \texttt{jabit-cryptography-sc} instead of \texttt{jabit-cryptography-bc}.
  ...
  \attachlisting{snippets/context.java}
  ...
  \attachlisting{snippets/identity.java}

  and add a contact

  \attachlisting{snippets/contact.java}

  to which you can send a message

  \attachlisting{snippets/send.java}
...
\end{lstlisting}
  ...
  \attachlisting{snippets/application.properties}
